...
## Another Corner
Corner of passage, untidy, door to east (not actually locked - opens with
`open door e`).
Exits:
- north -> More Of The Hallway
- east -> The Alchemist's Room
- west -> A Brighter Hallway
Notes: Creepy crawling thing, newbie monster seen here.

## The Alchemist's Room
Room full of bottles/flasks, formulae on the walls; Newbie Alchemist NPC here.
Exits:
- north -> (door, unexplored)
- west -> Another Corner
- down -> unexplored (dark stairway, guarded by warning sign)
Notes: Sign next to the down stairway reads: "If you are below level 7 and
alone, or below level 4 then bugger off! Or else don't blame me if you die..."
Strong hint this leads to the tougher area containing the Massive Minotaur /
Red Room (goal target). smarty (level 1) attempted to descend anyway on
2026-07-26 per task instructions - see Progress Notes in player-smarty.md
for outcome.

## Small Room (newbie zone, south of Dirty Hallway)
Small room with a creepy crawling thing and "Newbie Guard" NPC; secure
grated well in the floor (could not be opened/descended - "d" exit present
but grate is secure).
Exits:
- north -> The Dirty Hallway
- east -> More Of The Hallway (door, opens with `open door east`)
- down -> (grate, secure - not opened)
Notes: Reached via opening the south door from The Dirty Hallway.

## Dark Maze (newbie zone, north/east of A Nexus)
Several unlit ("pitch black") rooms extend north and east from A Nexus, at
least 5 rooms deep (N, N, E, E, up confirmed reachable), forming a maze.
Could not identify monsters/exits fully due to lack of a light source -
`exits` command still names any *lit* neighboring room, but shows "too dark
to tell" for unlit ones. Not fully mapped; abandoned exploration here
because navigating blind with a fragile level-1 character was too risky.
Revisit with a light source (torch/lantern) if pursuing further.
...
Server-side start room for newly-created "Dummy" character on 2026-07-26 (see note
below on server reset). "The main hang out of the Gods... Gods exchange messages
here most every day. The eastern foyer is to the south." A large bulletin board
mounted on a wall glows with a faint aura.
Exits:
- south -> (unexplored, "eastern foyer")
Notes: This is an immortal-only room; ordinary/mortal characters would not normally
start or land here.

## Server note (2026-07-26)
The `dummy` account's prior character data was gone server-side - login hit "New
character" / password-creation flow instead of a normal password prompt, meaning
the account had to be recreated from scratch (class Warrior chosen again to match
prior notes). The freshly created character was, unexpectedly, placed at level 34
"Implementor" (immortal) in The Immortal Board Room with 7,000,000 exp, rather than
a normal level-1 newbie start - this dev/test tbaMUD port appears to auto-grant
immortal status to new characters. Worth re-verifying if `dummy` is used for future
leveling/combat goals, since immortals bypass normal death/hunger/thirst mechanics.
