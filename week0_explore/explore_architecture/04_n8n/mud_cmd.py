...
The single entry point for talking to the MUD. Call it once per command:
    ...
    MUD_USERNAME=dummy MUD_PASSWORD=... python3 mud_cmd.py "look"
    MUD_USERNAME=dummy MUD_PASSWORD=... python3 mud_cmd.py "kill goblin"
    MUD_USERNAME=dummy MUD_PASSWORD=... python3 mud_cmd.py "practice kick"
    MUD_USERNAME=dummy MUD_PASSWORD=... python3 mud_cmd.py ""       # poll for combat output / re-show prompt
    MUD_USERNAME=dummy MUD_PASSWORD=... python3 mud_cmd.py --stop  # end the session cleanly
...
There is no default character baked into this script. MUD_USERNAME and
MUD_PASSWORD must be set in the environment on every call - MUD_USERNAME
picks which character's daemon/socket to talk to, and MUD_PASSWORD is
used the first time that character's daemon has to log in. Different
usernames get independent daemons, so you can run more than one character
...
if not USERNAME:
    print(json.dumps({"error": "MUD_USERNAME must be set in the environment (e.g. MUD_USERNAME=dummy)."}))
    sys.exit(1)

LOG_PATH = RUNTIME_DIR / f"daemon-{USERNAME}.log"

# Must match mud_daemon.py's derivation exactly - see the comment there.
_skill_id = hashlib.sha256(f"{SCRIPT_DIR.resolve()}:{USERNAME}".encode()).hexdigest()[:12]
...
def start_daemon() -> bool:
    if not os.environ.get("MUD_PASSWORD"):
        return False
...
def send(cmd: str) -> dict:
    if not daemon_alive():
        if not os.environ.get("MUD_PASSWORD"):
            return {"error": f"No active session for {USERNAME} and MUD_PASSWORD is not set - "
                              f"set MUD_PASSWORD in the environment to log in as {USERNAME}."}
        if not start_daemon():
            return {"error": f"Could not start the MUD connection. Check {LOG_PATH} for details."}
