...
- I think we will not to implement a MCP
...
- I think most "garbage" tool output we hit will trace back to the transport/session layer (buffering, prompt-matching regexes, environment leaks) rather than to the LLM. So we should verify raw session state before attributing a bad result to the model.
...
- Found and fixed a real dispatch bug: the LLM called `look` with an empty target plus a preposition, which `Primitives.look` turned into the literal invalid command `"look at"`. Fixed one layer up, in `ToolCatalog::NORMALIZERS`, not in `Primitives` — the MUD-specific quirk lives below the generic transport.
...
- The baseline loop (tool registry + prompt builder + logging + REPL) covers mechanics but not strategy. It has no planning primitive, which is why the agent wanders on any objective more complex than "read this file." This directly confirms preweek's uncertainty about needing a specialized planning step, not just a specialized loop.
