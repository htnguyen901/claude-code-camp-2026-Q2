...
result = Boukensha::Session.play(goal: goal, player: player, max_turns: max_turns) do
  # Same room_knowledge tool boukensha_loader.rb registers for the regular
  # REPL/CLI path — optional, but the Player's prompt references it.
  tool "room_knowledge",
       description: "Check which items/mobs/NPCs/scenery in a room you've already " \
                     ...
                     "past exploration history.",
       parameters: {
         room_title: { type: "string", description: "The exact room title, as it appeared in your most recent look/move result." }
       } do |room_title:|
