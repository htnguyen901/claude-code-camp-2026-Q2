...
class TestPlayerCredentials < Minitest::Test
  ...
  # WORLD_MAP_PLAYER (docs/plans/world_knowledge/world_knowledge.md
  # decision 4) — same overlay mechanism as MUD_NAME/MUD_PASSWORD, matched
  # by key name against whichever server declares it (log-viz, not mud).
  def test_overlays_world_map_player_onto_a_server_that_declares_that_key
    servers = {
      "log-viz" => { command: "log_viz", args: ["--mcp"], prefix: "world", required: true,
                     env: { "WORLD_MAP_PLAYER" => "" } }
    }

    Boukensha.send(:overlay_player_credentials, servers, player)

    assert_equal "noir", servers["log-viz"][:env]["WORLD_MAP_PLAYER"]
  end

  def test_does_not_invent_world_map_player_for_a_server_that_never_declared_it
    servers = { "mud" => { command: "mud-manager", args: [], prefix: nil, required: true,
                           env: { "MUD_NAME" => "", "MUD_PASSWORD" => "" } } }

    Boukensha.send(:overlay_player_credentials, servers, player)

    refute servers["mud"][:env].key?("WORLD_MAP_PLAYER")
  end
end
