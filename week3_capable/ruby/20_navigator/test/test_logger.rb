...
class TestLogger < Minitest::Test
  ...
  # docs/plans/agent_loop/log_viz_visibility.md: log_viz's Iteration view
  # can't tell which task an entry belongs to unless every phase carries
  # `task`, not just `response` (which already had it via
  # execution_metadata). These five are the ones log_viz renders as
  # transcript entries.
  def test_request_carries_the_task_name
    logger = new_logger
    logger.request(payload: sample_payload, messages: sample_messages, tools: sample_tools, iteration: 1, task: "planner")

    event = read_events(logger).find { |e| e["phase"] == "request" }
    assert_equal "planner", event["task"]
  end

  def test_tool_call_carries_the_task_name
    logger = new_logger
    logger.tool_call(name: "tbamud__look", args: {}, task: "judge")

    event = read_events(logger).find { |e| e["phase"] == "tool_call" }
    assert_equal "judge", event["task"]
  end

  def test_tool_result_carries_the_task_name
    logger = new_logger
    logger.tool_result(name: "tbamud__look", result: "raw text", ok: true, task: "judge")

    event = read_events(logger).find { |e| e["phase"] == "tool_result" }
    assert_equal "judge", event["task"]
  end

  def test_reasoning_carries_the_task_name
    logger = new_logger
    logger.reasoning(text: "thinking...", task: "player")

    event = read_events(logger).find { |e| e["phase"] == "reasoning" }
    assert_equal "player", event["task"]
  end

  # This is Logger#plan's pre-existing tool-call-preamble event (the
  # model's inline reasoning before a tool call) — unrelated to
  # Tasks::Planner's own output, which travels as a normal request/response
  # pair under task: "planner" (see orchestrator.md §2's naming note).
  # Adding `task:` here just means a Judge's own preamble is attributable
  # too, not a reuse of the name for something new.
  def test_plan_preamble_carries_the_task_name
    logger = new_logger
    logger.plan(text: "about to look around", task: "player")

    event = read_events(logger).find { |e| e["phase"] == "plan" }
    assert_equal "player", event["task"]
  end

  def test_task_defaults_to_nil_when_omitted
    logger = new_logger
    logger.request(payload: sample_payload, messages: sample_messages, tools: sample_tools, iteration: 1)
