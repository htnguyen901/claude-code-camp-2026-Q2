...
class TestTasksJudge < Minitest::Test
  ...
  # docs/plans/agent_loop/capability/resource_bootstrap.md §5 — the Judge
  # must be told to notice a sub-goal that stays unresolved across several
  # *different* attempts (not just a literally repeated action), since the
  # deterministic repeated_actions tally can't catch that shape of stuck.
  def test_default_prompts_dir_includes_the_semantic_stuck_signal
    text = Judge.system_prompt({}, default_prompts_dir: Boukensha::Config::PROMPTS_DIR)
    assert_includes text, "different"
    assert_includes text, "replan"
  end
