...
class TestTasksBaseDefaultPrompts < Minitest::Test
  ...
  # docs/plans/agent_loop/capability/resource_bootstrap.md §2/§3 — general
  # resource-acquisition literacy (don't limit yourself to asking/buying) and
  # treating a silent non-reply as a failed attempt toward the three-strikes
  # rule, both worded generically rather than about any one mechanic.
  def test_player_default_system_prompt_includes_resource_acquisition_and_no_reply_guidance
    text = Player.system_prompt({}, default_prompts_dir: Boukensha::Config::PROMPTS_DIR)

    assert_includes text, "looting"
    assert_includes text, "no reply at all"
    refute_includes text, "hunger", "the guidance must stay generic, not hardcode this incident's specific resource"
  end
