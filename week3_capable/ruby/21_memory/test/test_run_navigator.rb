...
class TestRunNavigator < Minitest::Test
  ...
  # docs/plans/agent_loop/player_route_adherence.md §3a: the Navigator's
  # route description is a numbered, one-hop-per-line list rather than one
  # prose sentence, so a small model can mechanically follow it hop by hop.
  # This is a fixture asserting the shape a scripted 3-hop route comes back
  # in — the Navigator itself is just an LLM call, so the "numbered list" is
  # produced by the (scripted, here) model response, not by any code in
  # run_navigator; this pins the expected contract that prompts/navigator/
  # system.md's step 2 asks the model to follow.
  def test_a_known_route_is_described_as_a_numbered_list
    with_temp_boukensha_dir(NAVIGATOR_SETTINGS_YAML) do
      mcp = fake_mcp do |registry|
        registry.tool("world__route_to", description: "route", parameters: { from: { type: "string" }, to: { type: "string" } }) do |from:, to:|
          JSON.generate(hops: ["north", "east", "up"])
        end
      end

      numbered_route = "Route (3 hops) to The Reception:\n1. north\n2. east\n3. up"
      server = NavigatorScriptedServer.new([
        ollama_tool_use_response(name: "world__route_to", args: { from: "The Temple Square", to: "The Reception" }),
        ollama_text_response(numbered_route)
      ])

      result = run_navigator(server, mcp: mcp, to: "The Reception")
      server.stop

      assert_equal numbered_route, result
      assert_equal 3, result.lines.count { |line| line =~ /^\d+\. / }, "one numbered line per hop"
    end
  end
