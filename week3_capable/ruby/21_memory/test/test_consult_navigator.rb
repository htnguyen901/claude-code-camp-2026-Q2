...
class TestConsultNavigator < Minitest::Test
  ...
  # docs/plans/agent_loop/player_route_adherence.md §3b: dispatching
  # consult_navigator must set the caller's ctx.route to the Navigator's
  # reply, wrapped (via Context#effective_system) in the fixed follow-it
  # instruction — and neither should appear before a call happens. Stubs
  # Boukensha.run_navigator the same way
  # TestConsultNavigatorIsolationForJudgeAndPlanner does below, so this stays
  # a pure unit test of the tool wrapper's own effect on ctx, independent of
  # a real model round trip (already covered by test_run_navigator.rb).
  def test_dispatching_consult_navigator_sets_ctx_route_wrapped_in_the_follow_instruction
    original = Boukensha.method(:run_navigator)
    Boukensha.define_singleton_method(:run_navigator) { |**_| "Route (2 hops) to The Beginning Of The Passage:\n1. north\n2. east" }
    ...
    refute_includes ctx.effective_system, "Route (2 hops)"
    refute_includes ctx.effective_system, Boukensha::Context::ROUTE_INSTRUCTION

    registry.dispatch("consult_navigator", from: "The Temple Square", to: "The Beginning Of The Passage")

    assert_includes ctx.effective_system, "Route (2 hops) to The Beginning Of The Passage:\n1. north\n2. east"
