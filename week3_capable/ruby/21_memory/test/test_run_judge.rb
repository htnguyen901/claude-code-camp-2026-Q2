...
class TestRunJudge < Minitest::Test
  ...
  # docs/plans/agent_loop/capability/resource_bootstrap.md §5 — a sub-goal
  # stuck across several *different* attempts (here: different shops, same
  # refusal) has no repeated tool+args pair, so repeated_actions is empty
  # and the mechanical override in run_judge never fires. The prompt's new
  # semantic-stuck guidance asks the Judge itself to notice this and say
  # `replan` in its own text — this confirms that text still parses to
  # :replan, not overridden, when the LLM does exactly that.
  def test_a_same_subgoal_different_attempts_pattern_with_no_literal_repeats_parses_as_replan
    server = JudgeScriptedServer.new([
      ollama_text_response(
        "Several different shops have all refused the same purchase for the same reason; " \
        "this sub-goal is stuck even though no single action repeated.\nVERDICT: replan"
      )
    ])

    result = run_judge(server, mcp: fake_mcp, repeated_actions: {})
    server.stop

    assert_equal :replan, result[:verdict]
    refute result[:overridden], "the LLM's own VERDICT line should drive this, not the mechanical override"
  end
