...
require_relative "boukensha/tasks/navigator"
require_relative "boukensha/tasks/chronicler"
...
module Boukensha
  ...
  def self.run(
    ...
    register_navigator_tool(registry, connections, logger: logger, model: navigator_model, backend: navigator_backend,
  ...
  def self.repl(
    ...
    logger  = Logger.new(log: log, snapshot: {
      ...
      self_manage:            cfg.self_manage?,
      session_max_cost_usd:   cfg.session_max_cost_usd,
    ...
    # Safety net for any exit path that isn't /exit or a clean EOF (e.g. the
    # Interrupt rescued above) — flush_memory! itself resets @judge_memory
    # once it fires, so a boundary that already flushed (via /exit or EOF
    # inside Repl#start) sees nothing notable left and no-ops here.
    repl&.flush_memory!(:exit)
    logger&.close
  end
  ...
  # player_memory: this player's memory digest text (Boukensha::PlayerMemory
  #                #digest_text), or nil — see docs/plans/memory/
  #                player_memory.md decision 4. Memory reaches the Player
  #                only through the Planner, never as a second channel into
  #                the Player's own system prompt; nil (no memory feature,
  #                no player, or no digest written yet) omits the block
  #                entirely, byte-identical to pre-memory behavior.
  def self.run_planner(
    ...
    ctx.add_message(:user, planner_input(goal: goal, transcript_tail: transcript_tail, prior_plan: prior_plan, replan_reason: replan_reason, player_memory: player_memory))
  ...
  # memory digest — see docs/plans/memory/player_memory.md §3. Mirrors
  # run_planner's shape exactly, minus tools: a small Agent#run loop over a
  # throwaway Context/Registry built from `tasks.chronicler.tools`
  # (deny-by-default with no such block configured, per Tasks::Base.
  # tool_policy). Unlike every other run_* sibling, there is no mcp:
  # parameter at all — the Chronicler is never handed the session's live MCP
  # connections, so it has nothing to dispatch even if a tool somehow got
  # past tool_policy (decision 3: tools are just tools, not memory).
  #
  # goal/outcome:  this session's objective and how it ended, as plain text
  #                (see Session.play's outcome: computation).
  # checkpoints:   an Array of JudgeMemory::Entry (or anything responding to
  #                turn/verdict/reasoning/overridden) — plain text is enough
  #                here, same "consumed only by a human/LLM-readable block,
  #                never branched on" posture planner_input/judge_input
  #                already use.
  # prior_digest:  this player's existing memory digest text, or nil for a
  #                brand new character's first-ever session.
  # logger:        required, like run_planner's/run_judge's — every real
  #                call site (Boukensha::Session) already has one to hand it.
  #
  # Returns plain digest text; the caller is responsible for handing it to
  # Boukensha::PlayerMemory#save_digest.
  ...
  # goal/outcome/checkpoints/prior_digest assembled into the one user
  # message the Chronicler sees — plain text is enough here, same posture
  ...
  #                   independent of what any other task's policy allows.
  #                   Lets the Judge dispatch the same world__room_knowledge
  #                   tool over the same live MCP connections (no second
  #                   login), while still running its
  ...
  # Session-wide cost governor for the self-managed continuation loop — see
  # docs/plans/agent_loop/self_management.md §1. `logger` is the one Logger
  # instance shared across every task (Player, Planner, Judge, Navigator)
  # for the whole session, so `logger.total_cost_usd` is already a true
  # all-tasks total with no extra plumbing. A free function over
  # (logger, cfg), not a class, mirroring Session.checkpoint?'s own shape.
  #
  # Returns :ok, :warn (spend has crossed cfg.session_cost_warn_pct of the
  # budget — one more, wind-down turn before stopping), or :exhausted (spend
  # has met or passed the budget — stop before starting another turn at
  # all). cfg.session_max_cost_usd nil/0 disables the budget: always :ok.
  ...
  # Walks from `from` to `to` over already-discovered rooms/exits — see
  # docs/plans/agent_loop/navigator.md. Mirrors .run_judge's shape exactly: a
  # throwaway Context/Registry built from `tasks.navigator.tools`, registered
  # against the session's already-connected MCP servers (no second MUD
  # login), run through a real Agent#run loop so it can dispatch
  # world__route_to/tbamud__move. Unlike run_judge, there's no verdict-style
  # parsing of the result — like run_planner, the returned text is consumed
  # only as the caller's tool-result string, never branched on here.
  #
  # from/to:  exact room titles, as supplied by the calling Player (see
  #           navigator.md §3 — Navigator trusts these the same way
  #           world__route_to's own from:/to: params already do).
  # mcp:      the session's Boukensha::McpConnections, or nil for an empty
  #           registry (zero tools) — same contract as run_judge's mcp:.
  # logger:   required, like run_judge's — every real call site already has
  #           one to hand it.
  ...
  # Registers the native `consult_navigator` tool onto a task's own
  # registry — see docs/plans/agent_loop/navigator.md §5 and
  # docs/plans/agent_loop/agents_tools/tool_scope_rework.md §5. Not an MCP
  # tool (no external server involved): a plain registry.tool(...) block,
  # called at every call site that builds a registry that should be able to
  # reach it (Player: .run/.repl/Session.play; Judge/Planner: .run_judge/
  # .run_planner), right after that registry's logger exists (the block
  # closes over `logger`, needed for the eventual .run_navigator call).
  # Because Tasks::Navigator never mutates world state, handing this to
  # Judge/Planner too costs nothing beyond a read-only round trip, same as
  # calling world__room_knowledge directly.
  #
  # Gated by the calling task's own `tools:` policy exactly like any other
  # tool name — Registry#tool already no-ops (denies, doesn't raise) for a
  # name outside policy, so a task whose `tasks.<name>.tools` doesn't add an
  # `allow: [consult_navigator]` entry simply never sees this tool at all.
  def self.register_navigator_tool(
    ...
    registry.tool(
      ...
      description: "Ask a route-finding specialist whether there's a known " \
                   ...
                   "path it describes.",
      parameters: {
        from: { type: "string", description: "The current room title, exactly as your last look/move result showed it." },
        to:   { type: "string", description: "The destination room title, exactly as you've seen it (e.g. in an exit list)." }
      }
    ) do |from:, to:|
      route = run_navigator(from: from, to: to, mcp: connections, logger: logger,
                             model: model, backend: backend, api_key: api_key, ollama_host: ollama_host)
      # Durable, checkable route state for the caller's own Context — see
      # docs/plans/agent_loop/player_route_adherence.md §3b. Every call
      # overwrites whatever was there before: a fresh consult always
      # supersedes a stale one. Lives in @system via Context#effective_system,
      # never in @messages, so it survives compaction the same way ctx.plan
      # already does.
      registry.context.route = route
      route
    end
  end
