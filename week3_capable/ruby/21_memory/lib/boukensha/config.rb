...
module Boukensha
  class Config
    ...
    # ---------- session-wide autonomy (self-managed continuation) ---------
    # Reads `session:` — docs/plans/agent_loop/self_management.md. Unlike
    # planner_enabled?/judge_enabled?, self_manage? defaults OFF: it changes
    # the execution model itself (the Player keeps acting, unattended, turn
    # after turn) rather than just adding an advisory round trip, so it
    # follows compactor_enabled?'s evidence-gated posture, not
    # planner_enabled?'s "flip straight to on".
    ...
    # Session-wide spend cap across every task (Player + Planner + Judge +
    # Navigator) combined — nil/0 disables it entirely.
    def session_max_cost_usd
      v = dig(:session, :max_cost_usd)
      v.nil? || v == 0 ? nil : Float(v)
    end

    # Fraction of session_max_cost_usd that triggers a one-turn wind-down
    # before the self-managed loop stops for good.
    ...
    # ---------- Player memory -----------------------------------------------
    # Reads `memory.enabled`. Defaults OFF — an LLM-authored digest that's
    # wrong doesn't just waste tokens, it can steer a future session's plan
    # in the wrong direction, so this stays opt-in-until-evaluated the same
    # way compactor_enabled? does. See docs/plans/memory/player_memory.md
    # decision 5.
    def memory_enabled?
