...
module Boukensha
  class Logger
    ...
    attr_reader :session_id, :path, :total_cost_usd
    ...
    #
    # task: — see docs/plans/agent_loop/log_viz_visibility.md. Historically
    # only #response carried this (via execution_metadata); log_viz's
    # Iteration view can't tell which task a request/tool_call/reasoning/
    # preamble entry belongs to without it too.
    def request(payload:, messages:, tools:, iteration:, wrap_up: false, task: nil)
      write_log(
        phase:          "request",
        iteration:      iteration,
        wrap_up:        wrap_up,
    ...
    def tool_call(name:, args:, task: nil)
      write_log(phase: "tool_call", name: name, args: args, task: task_name(task))
    end
    ...
    def tool_result(name:, result:, ok: true, error: nil, compacted: nil, task: nil)
      write_log(phase: "tool_result", name: name, result: result.to_s, ok: ok, error: error,
                compacted: compacted&.to_s, task: task_name(task))
    end
    ...
    def reasoning(text:, redacted: false, task: nil)
      write_log(phase: "reasoning", text: text.to_s, redacted: redacted, task: task_name(task))
    end

    def plan(text:, task: nil)
      write_log(phase: "plan", text: text.to_s.strip, task: task_name(task))
    end

    # The Judge's parsed continue/replan/flag decision — a distinct event
    # from #response (the Judge's raw model output, already logged by
    # Agent#run under task: "judge") so a viewer can find "what did the
    # Judge decide" without re-parsing response text. See
    # docs/plans/agent_loop/evaluator.md §5-§6.
    #
    # overridden: true when the deterministic repeated-action guard
    # replaced the LLM's own :continue verdict (see
    # docs/plans/agent_loop/evaluator_judge_redesign.md §6) — lets
    # log_viz/OTEL distinguish "the Judge itself decided this" from "the
    # mechanical guard decided this", which matters for comparing Judge
    # verdicts against human review.
    def judge_verdict(verdict:, text:, task: nil, overridden: false)
      write_log(phase: "judge_verdict", verdict: verdict.to_s, text: text.to_s.strip, overridden: overridden, task: task_name(task))
    end
    ...
    def execution_metadata(task:, backend:, usage:)
      ...
      @total_cost_usd += cost_usd if cost_usd
      metadata = {
        task: task_name(task),
