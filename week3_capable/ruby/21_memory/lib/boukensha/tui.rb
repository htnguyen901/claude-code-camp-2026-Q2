  ...
  class Tui
    ...
    def render_progress
      if @live[:active]
        ...
        lip(:cyan).render(
          "#{frame} #{action}  (iter #{iter}/#{max} · #{secs}s · ↑ #{itok} · ↓ #{otok} · #{calls} calls)#{autonomy_suffix}"
        )
      else
        ...
        lip(color).render(
          "  [ready]   ctx #{used} / #{max} (#{pct}%)   #{turns} turns#{autonomy_suffix}"
        )
      end
    end
    ...
    def ctx_color(pct)
      if pct >= CTX_ALERT_PCT || cost_status == :exhausted
        :red
      elsif pct >= CTX_WARN_PCT || cost_status == :warn
        :yellow
      else
        :bright_black
      end
    ...
    # Extends the live-progress line with the self-managed run's state and
    # running spend — see docs/plans/agent_loop/self_management.md §2.
    # @repl.autonomy_state is the sole source of truth; Tui holds no
    # autonomy state of its own.

    def autonomy_suffix
      parts = [autonomy_label, cost_label].reject(&:empty?)
      parts.empty? ? "" : "  #{parts.join("  ")}"
    end

    def autonomy_label
      case @repl.autonomy_state
      when :running then "●auto turn #{@turn_count}"
      when :paused   then "⏸paused turn #{@turn_count}"
      else ""
      end
    end

    def cost_label
      budget = Boukensha.config.session_max_cost_usd
      return "" unless budget && budget.positive?

      spent  = @repl.logger.total_cost_usd
      prefix = cost_status == :ok ? "" : "⚠ "
      format("%s$%.2f/$%.2f", prefix, spent, budget)
    end

    def cost_status
      Boukensha.session_budget_status(@repl.logger, Boukensha.config)
    end
    ...
    def submit_input
      ...
      elsif %i[running paused].include?(@repl.autonomy_state)
        @conversation << "(self-managed run in progress — /pause or /stop first)"
        @dirty = true
      else
        @conversation << "> #{input}"
        @dirty = true
