...
module Boukensha
  module Tools
    ...
    module Mcp
      ...
      # Spawn-only half of #register — connecting is independent of any
      # task/registry/policy. Boukensha::McpConnections is the one-per-session
      # caller of this; #register (below) also uses it for the handful of
      # ad hoc/one-registry call sites (examples, single-registry tests) that
      # don't need McpConnections' multi-task sharing.
      def self.connect(command:, args: [], env: {})
        Boukensha::Mcp::Client.spawn(command: command, args: args, env: env)
      end

      def self.register(registry, command:, args: [], env: {}, prefix: nil)
        client = connect(command: command, args: args, env: env)
