...
module Boukensha
  module Backends
    ...
    class Base
      ...
      # Canonical provider key — matches the backend: symbol boukensha.rb's
      # `case backend` dispatches on (:anthropic, :openai, :gemini, :ollama,
      # :ollama_cloud) and what log_viz's PROVIDER_KEYS/CACHE_KEYS are keyed
      # by. Deliberately explicit per subclass rather than derived from the
      # Ruby class name — a generic CamelCase->snake_case conversion of
      # `OpenAI` produces "open_ai", not "openai" (see docs/bugs).
      def self.provider_name
        raise NotImplementedError, "#{self} must define .provider_name"
      end
