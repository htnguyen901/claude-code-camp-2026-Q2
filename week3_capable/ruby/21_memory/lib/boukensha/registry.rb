...
module Boukensha
  class Registry
    # Exposed so a tool wrapper built around a Registry (e.g.
    # register_navigator_tool — docs/plans/agent_loop/
    # player_route_adherence.md §3b) can reach the underlying Context to set
    # state like ctx.route, without every such wrapper needing its own ctx:
    # kwarg threaded in alongside registry:.
    attr_reader :context
