...
class TestMcpServersConfig < Minitest::Test
  ...
  def test_required_server_that_fails_to_spawn_raises
    ...
    config_from(yaml) do |cfg|
      err = assert_raises(RuntimeError) do
        Boukensha::McpConnections.connect(cfg)
      end
  ...
  def test_optional_server_that_fails_to_spawn_warns_and_continues
    ...
    config_from(yaml) do |cfg|
      ...
      out, err = capture_io do
        connections = Boukensha::McpConnections.connect(cfg)
      end
  ...
  def test_returns_a_tool_count_per_server
    ...
    config_from(server_yaml("mud", extra: "    prefix: tbamud")) do |cfg|
      ...
      connections = Boukensha::McpConnections.connect(cfg)
      connections.register(registry)
      # Every MudManager::Primitives method (56, reflected — see
      # mud_manager/tool_catalog.rb) plus the four session-lifecycle tools
      # (connect/disconnect/create_character/delete_character) the daemon
      # adds on top so one process can hold more than one logged-in
      # character — see mcp_server.rb#session_lifecycle_tools.
      assert_equal({ "mud" => 60 }, connections.summary)
    end
  ensure
    @fake&.stop
  end

  # docs/plans/tools_policy/mcp_connection_sharing.md's core guarantee: a
  # single McpConnections#connect spawns one subprocess per server, and
  # #register can be called any number of times (once per task) against two
  # independently-policied registries without spawning or handshaking again
  # — each call just re-filters the same already-fetched catalog.
  def test_registering_the_same_connections_twice_applies_each_registrys_own_policy_independently
    @fake = start_fake_mud
    config_from(server_yaml("mud", extra: "    prefix: tbamud")) do |cfg|
      connections = Boukensha::McpConnections.connect(cfg)

      full_ctx, full_registry = new_registry
      connections.register(full_registry)

      restrictive_ctx, restrictive_registry = new_registry(policy: Boukensha::ToolPolicy.new(allow: %w[tbamud__look]))
      connections.register(restrictive_registry)

      assert_equal 60, full_ctx.tools.size
      assert_equal %w[tbamud__look], restrictive_ctx.tools.keys
      assert_match(/You do: look/, restrictive_registry.dispatch("tbamud__look", {}))
    end
  ensure
    @fake&.stop
  end
