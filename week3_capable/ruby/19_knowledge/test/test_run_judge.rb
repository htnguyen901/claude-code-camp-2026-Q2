...
# Context/Registry, registered from the session's shared MCP connections
# and filtered through the Judge's own role: inspector policy.
class TestRunJudge < Minitest::Test
  ...
  # shared MCP connections would offer a broader catalog — Registry#tool
  # still silently drops anything not on the inspector list, so a scripted
  # attempt to call a denied tool comes back denied rather than actually
  # running the block.
  def test_a_tool_outside_the_configured_role_is_never_dispatchable_by_the_judge
    with_temp_boukensha_dir(JUDGE_SETTINGS_YAML) do
      ...
      refute attacked, "the Judge's role: inspector policy must deny tbamud__attack even though the shared connections offer it"
      assert_equal :flag, result[:verdict]
    end
  end
