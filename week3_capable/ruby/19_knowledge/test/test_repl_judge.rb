...
class TestReplJudge < Minitest::Test
  ...
  # docs/plans/agent_loop/evaluator_judge_redesign.md §3/acceptance
  # criteria: /clear must reset JudgeMemory the same turn it resets
  # @turns_since_checkpoint/@planned — a pre-/clear verdict must not leak
  # into the judge_input of a post-/clear checkpoint.
  def test_clear_resets_judge_memory_so_no_pre_clear_history_leaks_into_the_next_checkpoint
    player_server = ReplJudgeScriptedServer.new([
      ollama_text_response("Turn 1 done."),
      ollama_text_response("Turn 2 done.")
    ])
    ...
    assert_equal 2, judge_server.captured_bodies.size
    second_request = JSON.parse(judge_server.captured_bodies.last)
    user_text = second_request["messages"].find { |m| m["role"] == "user" }["content"]
    refute_includes user_text, "previous judgements",
                    "JudgeMemory must be reset on /clear — a pre-clear verdict must not leak into the post-clear checkpoint"
  end
