...
class TestSession < Minitest::Test
  ...
  # docs/plans/agent_loop/evaluator_judge_redesign.md's acceptance criteria:
  # the Judge stub keeps saying VERDICT: continue at every checkpoint while
  # the Player repeats the identical tool call. The mechanical override (not
  # the LLM) must first force a :replan once the repeat count crosses the
  # threshold, and then — if the same pattern persists against the new plan
  # — escalate to :flag instead of silently replanning forever.
  def test_repeated_actions_mechanically_override_continue_to_replan_then_escalate_to_flag
    with_temp_boukensha_dir("agent:\n  max_iterations: 1\n") do
      Dir.mktmpdir do |dir|
        log_path = File.join(dir, "session.jsonl")

        planner_server = SessionScriptedServer.new([
          ollama_text_response("1. Ask around for food."),                          # initial plan
          ollama_text_response("1. The begging isn't working; try the market instead.") # replan
        ])
        # 4 turns, each: one "look" tool_use (forces a wrap-up under
        # max_iterations: 1) + the wrap-up call itself.
        player_server = SessionScriptedServer.new(
          Array.new(4) { [ollama_tool_use_response(name: "look"), ollama_text_response("(wrap up) still exploring")] }.flatten
        )
        # The LLM Judge itself never says anything but :continue — any
        # :replan/:flag the driver ends up with must come from the
        # deterministic guard, not the model.
        judge_server = SessionScriptedServer.new(
          Array.new(4) { ollama_text_response("Still exploring, no concerns.\nVERDICT: continue") }
        )
        ...
        assert_equal 4, judge_server.captured_bodies.size, "one checkpoint per turn, including the turn that ends the session"
        assert_equal 2, planner_server.captured_bodies.size, "initial seed + exactly one replan (never a second one)"

        events = File.readlines(log_path).map { |l| JSON.parse(l) }
        verdicts = events.select { |e| e["phase"] == "judge_verdict" }
        assert_equal 4, verdicts.size
        assert_equal [%w[continue false], %w[continue false], %w[replan true], %w[flag true]],
                     verdicts.map { |v| [v["verdict"], v["overridden"].to_s] }
      end
    end
  end
