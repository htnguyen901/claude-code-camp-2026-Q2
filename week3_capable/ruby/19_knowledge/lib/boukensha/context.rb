...
module Boukensha
  class Context
    ...
    attr_accessor :current_tokens, :plan
    ...
    # Planner has run, the current plan appended as its own block. Every
    # backend must read this instead of `system` directly — see
    # docs/plans/agent_loop/orchestrator.md §2. nil/blank plan is a no-op so
    # orchestration-disabled sessions get byte-identical payloads.
