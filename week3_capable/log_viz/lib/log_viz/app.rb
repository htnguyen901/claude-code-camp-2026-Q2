...
module LogViz
  class App < Sinatra::Base
    ...
    helpers do
      ...
      # Task-section markers in the Iteration view transcript
      # (docs/plans/agent_loop/log_viz_visibility.md) — colored by the
      # task's position in `session.task_names`, the same index-cycled-
      # palette pattern `LIVE_MARKER_COLORS` already uses for map markers
      # (below/`app.rb`'s live-map code), so a brand-new task name (Judge,
      # a future subagent, ...) gets a distinct color automatically, with no
      # hardcoded name-to-color mapping to maintain.
      def task_color(task, session)
        names = session.task_names
        idx   = names.index(task) || 0
        LIVE_MARKER_COLORS[idx % LIVE_MARKER_COLORS.length]
      end

      def task_marker(task, session)
        return "" if task.to_s.empty?

        color = task_color(task, session)
        %(<div class="task-marker" style="--task-color: #{color}"><span class="task-marker-dot"></span>#{text_html(task)}</div>)
      end
