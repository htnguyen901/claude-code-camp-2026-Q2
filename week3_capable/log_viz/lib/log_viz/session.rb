...
module LogViz
  ...
  class Session
      ...
      File.foreach(@path) do |line|
        ...
        when "reasoning"
          @entries << Entry.new(type: :reasoning, text: event["text"],
                                 redacted: event["redacted"], task: event["task"],
                                 turn: current_turn, iteration: current_iteration)
        when "plan"
          @entries << Entry.new(type: :plan, text: event["text"], task: event["task"],
                                 turn: current_turn, iteration: current_iteration)
        when "response"
          usage = event["usage"]
          if usage
    ...
    # Every distinct task name that appears anywhere in this session, in
    # first-appearance order — not just from response events (@usage_series)
    # any more, now that request/tool_call/tool_result/reasoning/plan carry
    # `task` too (log_viz_visibility.md). Order matters: task_marker/
    # task_color (app.rb) index into this list to assign each task a stable
    # color for the session, the same index-cycled-palette pattern
    # LIVE_MARKER_COLORS already uses elsewhere.
    def task_names = @entries.map(&:task).compact.uniq
