module Boukensha
  class Agent
    ...
    def initialize(context:, registry:, builder:, client:, logger: Logger.new, compactor: Compactor.new, task_name: nil,
    ...
    def run
      ...
      loop do
        ...
        @logger.request(payload: payload, messages: @context.messages, tools: @context.tools, iteration: @iteration)

        response = @client.call(payload: payload, **call_opts)
    ...
    def wrap_up(reason)
      ...
      @logger.request(payload: payload, messages: @context.messages, tools: [], iteration: @iteration, wrap_up: true)
      response    = @client.call(payload: payload, tools: [], max_output_tokens: WRAP_UP_OUTPUT_TOKENS)
    ...
    def handle_tool_calls(content, response)
      ...
      tool_calls.each do |block|
        ...
        @logger.tool_result(name: name, result: result, ok: ok, error: error_message, compacted: compacted)
